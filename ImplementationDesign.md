...
- [ ] 革命（4枚カルテット → 強弱逆転・逆革命）
- [ ] 8切り（8を出したら場リセット・同プレイヤー続行）
- [ ] スペ3返し（ジョーカー1枚出しに♠3で対抗）
- [ ] 反則あがり判定（最強カード・8切り・スペ3返し・ジョーカー）
- [ ] 反則あがり時の処理（最下位化・ゲーム継続）
- [ ] 各ルールの動作確認
...
- [ ] NPC プレイヤーの管理（サーバー側で自動操作）
- [ ] NPC の基本戦略（出せる最小のカードを出す・出せなければパス）
- [ ] NPC の階級交換（大富豪時：最小カードを渡す）
